(function() {
  ...
  _.each = function(collection, iterator, context) {
    if (Array.isArray(collection)) {
      ...
      let allKeys = Object.keys(collection);
      for (var j = 0; j < allKeys.length; j+=1) {
        iterator(collection[allKeys[j]], allKeys[j], collection)
      }
    }
  };
...
*/
  ...
  _.filter = function(collection, test) {
    ...
    _.each(collection, function(elem, index, collection) {
      if (test.call(null, elem, index, collection)) {
  ...
  _.reject = function(collection, test) {
    ...
    return _.filter(collection, function(elem, index, collection) {
      return !test.call(null, elem, index, collection);
    }, context);
  };
/*
  let result = [];
...
/*    if (iterator === undefined) {
  ...
  _.memoize = function(func) {
    ...
    return function() {
      let stringedArg = JSON.stringify(arguments);

      if (result[stringedArg]) {
        return result[stringedArg];
      } else {
        let computation = func.apply(this, arguments);
        result[stringedArg] = computation;
        return computation;
      }
    }
  };
  ...
  _.shuffle = function(array) {
    ...
    let getRandomIndexFromArrCopy = function () {
      return Math.floor(Math.random() * arrCopy.length);
    };

    while (result.length < array.length) {
      let randomIndex = getRandomIndexFromArrCopy();
